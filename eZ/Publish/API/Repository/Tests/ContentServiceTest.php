...
class ContentServiceTest extends BaseContentServiceTest
{
    ...
    public function testCopyNonTranslatableFieldsFromPublishedVersionToDraft(): void
    {
        ...
        $typeCreate = $contentTypeService->newContentTypeCreateStruct('nontranslatable');
        ...
        $typeCreate->creatorId = $this->generateId(
            'user',
            $permissionResolver->getCurrentUserReference()->getUserId()
        );
        ...
        $publishedContent = $this->contentService->publishVersion($contentDraft->getVersionInfo());

        // Creating a draft in ger-DE language with the only field updated being 'title'
        $gerDraft = $this->contentService->createContentDraft($publishedContent->contentInfo);
        ...
        $gerContent = $this->contentService->updateContent($gerDraft->getVersionInfo(), $contentUpdateStruct);

        // Updating non-translatable field in eng-US language (allowed) and publishing it
        $engContent = $this->contentService->createContentDraft($publishedContent->contentInfo);
        ...
        $engContent = $this->contentService->updateContent($engContent->getVersionInfo(), $contentUpdateStruct);
        $this->contentService->publishVersion($engContent->getVersionInfo());

        // Publishing ger-DE draft with the empty non-translatable field
        $this->contentService->publishVersion($gerContent->getVersionInfo());

        // Loading main content
        $mainPublishedContent = $this->contentService->loadContent($engContent->id);
