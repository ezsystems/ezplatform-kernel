...
class ContentMapper
{
    ...
    public function getFieldValueForCreate(FieldDefinition $fieldDefinition, ?Field $field): Value
    {
        if (isset($field)) {
    ...
    public function getFieldValueForUpdate(?Field $newField, ?Field $previousField, FieldDefinition $fieldDefinition, bool $isLanguageNew): Value
    {
        $isFieldUpdated = isset($newField);
    ...
    public function getFieldsForUpdate(array $updatedFields, Content $content): array
    {
        ...
        foreach ($updatedFields as $updatedField) {
            ...
            $updatedFieldValue = $this->getFieldValueForUpdate(
                ...
                !in_array($updatedField->languageCode, $content->versionInfo->languageCodes)
            );

            if (!empty($field)) {
                $updatedFieldHash = md5(serialize($fieldType->toHash($updatedFieldValue)));
                $contentFieldHash = md5(serialize($fieldType->toHash($field->value)));
    ...
    public function getFieldsForCreate(array $createdFields, ContentType $contentType): array
    {
        ...
        foreach ($createdFields as $createdField) {
            ...
            $createdFieldHash = md5(serialize($fieldType->toHash($createdFieldValue)));
            $defaultFieldHash = md5(serialize($fieldType->toHash($fieldDefinition->defaultValue)));
