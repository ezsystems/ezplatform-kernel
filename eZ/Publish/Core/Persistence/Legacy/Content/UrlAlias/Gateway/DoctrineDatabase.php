...
final class DoctrineDatabase extends Gateway
{
    ...
    public function updateRow(int $parentId, string $textMD5, array $values, ?int $languageId = null): void
    {
        $query = $this->connection->createQueryBuilder();
        ...
        if (null !== $languageId) {
            $query->andWhere(
                $query->expr()->gt(
                    $this->dbPlatform->getBitAndComparisonExpression(
                        'lang_mask',
                        $query->createNamedParameter($languageId, ParameterType::INTEGER, ':lang_mask')
                    ),
                    0
                )
            );
    ...
    public function loadRow(int $parentId, string $textMD5, ?int $languageId = null): array
    {
        $query = $this->connection->createQueryBuilder();
        $query->select('*')->from(
            $this->connection->quoteIdentifier($this->table)
    ...
    public function loadAutogeneratedEntry(string $action, ?int $parentId = null, ?int $languageId = null): array
    {
        $query = $this->connection->createQueryBuilder();
        $query->select(
            '*'
