...
class Handler implements UrlAliasHandlerInterface
{
    ...
    private function internalPublishUrlAliasForLocation(
        ...
        while (true) {
            ...
            $row = $this->gateway->loadRow($parentId, $newTextMD5, $languageId);

            // If nothing was returned insert new entry
            if (empty($row)) {
                // Check for existing active location entry on this level and reuse it's id
                $existingLocationEntry = $this->gateway->loadAutogeneratedEntry($action, $parentId, $languageId);
    ...
    public function lookup($url, ?string $languageCode = null)
    {
        ...
        $languageMask = $this->maskGenerator->generateLanguageMaskFromLanguageCodes([$languageCode], true);

        $data = $this->gateway->loadUrlAliasData($urlHashes, $languageMask);
        if (empty($data)) {
